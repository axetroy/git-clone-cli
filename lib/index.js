...
const npmPackage = require(path.join(__dirname, '../npmPackage.json'));
  ...
  .action(function (repoUrl, command) {
    parseRepo(repoUrl || '', function (repo) {
      ...
      co(function *() {
        ...
        const hasExistDistDir = yield stat(paths.dist)
          .then(() => {
        ...
        if (hasExistDistDir) {
          ...
          wantReplace ? yield removeDir(paths.dist) : yield Promise.reject();
        }
        ...
        log.info(`clone repo ${repo.href.blue.underline} ...\n`);
